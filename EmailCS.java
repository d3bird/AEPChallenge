...
public class EmailCS {
	...
	String getFeedback(customer in) {
		if(in.getFeedback().isEmpty()) {
			...
			String output ="Thank you for your feedback! We'll be looking into what you have to say.";
			return output;
		}
	}
	...
	String getMonthlymessage(customer in) {
		if(in.getMonthlydata().isEmpty()) {
			...
			if(data.size()>=2){//more than one months
				...
				if(AdverageRate < 0) {
					line6="Your payments have been going down steadly, on average "+ (AdverageRate*-1) +" per month";
				}
				line6="";
				...
				if(Double.parseDouble(adverage)>Double.parseDouble(costPM)) {
					line1 ="You paid less than the average person, saving a total of "+(Double.parseDouble(adverage)-Double.parseDouble(costPM))+" dollars";
				...
				if(Double.parseDouble(costPM)<Double.parseDouble(costLM)) {
					...
					line3="It seems that you used more than the previous months, but all is not lost. You can do it!";
				...
				if(ontime) {
					line4 ="You have paid all of your bills on time! Keep up the good work!";
				}else {
				...
				if(Double.parseDouble(adverage)>Double.parseDouble(billamount)){
					line1 ="You paid "+billamount+ "dollars, and the average person paid "+adverage;
					line2 ="You saved a total of "+(Double.parseDouble(adverage) -Double.parseDouble(billamount))+ " dollars";
				}else {
					line1="Last month your bill was "+data.get(0).getBilledamount();
					line2="";
				}
	...
	String getouttageStuff(customer in) {
		...
		if(outageData.isEmpty()) {
			if(poleData.isEmpty()) {
				line1 ="We have no information at this moment about the power poles in your area, but we will keep you informed if something changes";
			}else {
			...
			if(poleData.isEmpty()) {
				line1 ="We have experienced a couple of power outages over the past couple months; however, we are working very hard to fix these problems and apologize for any inconveniences it might have caused.";
			}else {
				double outTime =0;
				double improvments =0;
				...
				line1 ="Since you have been a customer, we have experienced "+outTime+" of powerouttages, but we have made "+improvments+ " improvements to different power poles to reduce outage time in the future.";
			}
		}
		output =line1;
		return output;
